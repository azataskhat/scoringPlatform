...
Платформа мониторинга безопасности IoT-устройств на основе открытых источников разведки (OSINT).
Система автоматически собирает данные об IoT-устройствах из нескольких OSINT-сервисов (Shodan, Censys, GreyNoise, NVD),
оценивает качество каждого источника по скоринговой модели и визуализирует результаты через веб-интерфейс.

## Какую проблему решает?

IoT-устройства часто остаются незащищёнными и доступными из интернета. При этом данные о них разбросаны по множеству OSINT-источников, каждый из которых имеет разную степень полноты, актуальности и достоверности. Платформа:

- **Агрегирует** данные из 4 OSINT-источников в единую базу
- **Оценивает качество** каждого источника по 4 критериям (надёжность, своевременность, полнота, доступность)
- **Визуализирует** устройства на карте, уязвимости в таблицах, тренды на графиках
- **Обновляет данные автоматически** по расписанию (от 30 до 360 минут в зависимости от источника)

## Для кого?

- Специалисты по информационной безопасности
- Исследователи в области IoT-безопасности
- Аналитики SOC/CERT-команд
- Студенты и преподаватели в области кибербезопасности

## Основные возможности

- Автоматический сбор данных из Shodan, Censys, GreyNoise и NVD (NIST)
- Количественная оценка качества OSINT-источников (скоринг)
- Интерактивная карта IoT-устройств с геолокацией (Leaflet + PostGIS)
- Дашборд с метриками и трендами (Recharts)
- Таблица уязвимостей с фильтрацией по severity/CVSS
- Генерация отчётов
...
```
├── backend/                 # Java Spring Boot (WebFlux + R2DBC)
│   ├── src/main/java/       # Контроллеры, сервисы, модели, репозитории
│   └── src/main/resources/  # Конфигурация, SQL-схемы
├── data_collector/          # Python-коллектор OSINT-данных
│   ├── collectors/          # Модули для каждого OSINT-источника
│   ├── scheduler.py         # Планировщик задач (APScheduler)
│   └── config.yaml          # Настройки интервалов и запросов
├── frontend/                # React + TypeScript SPA
│   └── src/
│       ├── components/      # UI-компоненты (карта, графики, таблицы)
│       ├── pages/           # Страницы приложения
│       ├── services/        # API-клиент (Axios)
│       └── hooks/           # React-хуки (SSE, API)
...
**Можно ли добавить новый OSINT-источник?**
Да. Создайте новый коллектор в `data_collector/collectors/`, унаследовав `BaseCollector`, и зарегистрируйте его в `scheduler.py`.

**Как изменить веса скоринговой модели?**
Через API: `PUT /api/scoring/weights` или в конфигурации `backend/src/main/resources/application.yml`.

**Нужны ли платные API-ключи?**
Shodan, Censys и GreyNoise предоставляют бесплатные тарифы с ограничениями. NVD API бесплатен, но требует регистрации для получения ключа.

**Какие порты сканируются?**
По умолчанию: 80, 443, 1883, 8883, 5683, 502 — типичные порты IoT-протоколов (HTTP, MQTT, CoAP, Modbus).
